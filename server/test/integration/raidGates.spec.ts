...
describe("raid start — pinned server state", () => {
  it("requires the current ruleset and an owned, unique roster", async () => {
    ...
    const foreign = await call<{ error: string }>("POST", "/raid/start", s.token, { raidId: 1, orderedUnitIds: ["not-owned"], rulesetVersion: 2 });
    expect(foreign.body.error).toBe("unit_not_owned");
    const duplicate = await call<{ error: string }>("POST", "/raid/start", s.token, { raidId: 1, orderedUnitIds: ["z1", "z1"], rulesetVersion: 2 });

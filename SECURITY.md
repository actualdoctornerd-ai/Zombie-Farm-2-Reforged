...
version 5, raid ruleset version 52). It covers authentication, sessions, the exclusive writer
...
- `/raid/start` pins the entire combat configuration from server-owned roster and catalog state
  ...
  ruleset 6 (current version 52) `raidVerifier.grabberOf` returns `null` unconditionally — hazards are client-only
  and are not simulated server-side at all.
- `/raid/finish` requires a matching `rulesetVersion` (`RAID_RULESET_VERSION = 52`; a mismatch

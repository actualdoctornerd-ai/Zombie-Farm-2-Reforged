...
**Rare zombies are a separate roll, with the same treatment.** All eleven raids independently roll a
rare zombie on a win (`src/raid/zombieDrops.ts`), on ONE rate ladder read off the raid's
...
their own pixel **Video Game Zombie** (an ordinary-category species) at the same **2.0%** ceiling. The four faction
...
same way from their own rung (the Aliens' Zastronaut 2 → **12%**).
